...
const getCartClosedFlag = gql`
  query ($cartId: String!) {
    getCartClosedFlag(cartId: $cartId) {
      flag
      phoneNo
    }
  }
...
function PlaceOrderPage(props) {
  ...
  const [phoneNoState, setPhoneNoState] = useState(null);
  ...
  const [cartClosedFlag, setCartClosedFlag] = useState();
  const [cartClosedFlag1, setCartClosedFlag1] = useState();
  useEffect(() => {
    console.log(props.getCartClosedFlag.getCartClosedFlag);

    if (props.getCartClosedFlag.getCartClosedFlag !== undefined) {
      setCartClosedFlag(props.getCartClosedFlag.getCartClosedFlag.flag);
      setCartClosedFlag1(props.getCartClosedFlag.getCartClosedFlag.flag);
      setPhoneNoState(props.getCartClosedFlag.getCartClosedFlag.phoneNo);
    }
  }, [props.getCartClosedFlag.getCartClosedFlag]);
  const placeOrderFunction = async () => {
    if (
      ...
      houseNo.current.value !== null &&
      colony.current.value !== "" &&
      colony.current.value !== null
    ) {
      const result = await props.placeOrder({
        variables: {
          name: name.current.value,
          phoneNo: phoneNoState,
  ...
  return (
    <div>
      <Dialog open={open2} aria-labelledby="responsive-dialog-title">
      ...
      <div style={{ textAlign: "center" }}>
        ...
        <TextField
          ...
          className={classes.textBox}
        />
        {cartClosedFlag !== undefined &&
          cartClosedFlag !== null &&
          cartClosedFlag == false && (
            <TextField
              required
              id="outlined-required"
              label="Phone Number"
...
export default compose(
  ...
  graphql(getCartClosedFlag, {
    name: "getCartClosedFlag",
    options: () => ({
      variables: {
